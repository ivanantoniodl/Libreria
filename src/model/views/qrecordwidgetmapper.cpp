...
*/
QRecordWidgetMapper::QRecordWidgetMapper(ModelBase *Model,
									   QWidget *parent) :
	QWidget(parent)
{
...
QRecordWidgetMapper::QRecordWidgetMapper(QWidget *parent):
	QWidget(parent)
{
	signalMapper = new QSignalMapper(this);
	model = NULL;
...
QRecordWidgetMapper::~QRecordWidgetMapper()
{
	qDeleteAll(lstConfMappers);
	lstConfMappers.clear();
}

void QRecordWidgetMapper::setModel(ModelBase *Model)
{
	model=Model;
...
void QRecordWidgetMapper::addValueMapper(QString recordProperty,
								   QVariant value)
{
...
*/
void QRecordWidgetMapper::addMapper(QString recordProperty,
								   QWidget *widget,
...
void QRecordWidgetMapper::addMultipleMapper(QStringList lstAtts, QWidget *widget)
{
...
*/
void QRecordWidgetMapper::removeMapper(QString recordProperty)
{
...
*/
void QRecordWidgetMapper::removeMapper(QWidget *widget)
{
	for ( int c=0; c<lstConfMappers.size(); c++ )
	{
		confMapper *con = lstConfMappers.at(c);
...
void QRecordWidgetMapper::toFirst()
{
	if ( newrecord )
	{
		cancelNewRecord();
...
void QRecordWidgetMapper::toLast()
{
	if ( newrecord )
	{
		cancelNewRecord();
...
void QRecordWidgetMapper::toNext()
{
...
void QRecordWidgetMapper::toPrevious()
{
	if ( pos <= 0 )
		return;
...
void QRecordWidgetMapper::toRecord(int index)
{
	if ( newrecord )
	{
		cancelNewRecord();
...
void QRecordWidgetMapper::toRecord(const QModelIndex & index)
{
	toRecord( index.row() );
}
...
*/
void QRecordWidgetMapper::mapper()
{
...
void QRecordWidgetMapper::emitNavigationSignals()
{
...
*/
QString QRecordWidgetMapper::getWidgetProperty(QWidget *widget)
{
...
*/
void QRecordWidgetMapper::cleanWidget(QWidget *widget)
{
	if ( widget->inherits("QLineEdit") ||
		 widget->inherits("QLabel")  )
...
void QRecordWidgetMapper::connectDataChanged(QWidget *widget)
{
	if ( widget->inherits("QLineEdit") )
	{
...
*/
TSqlError QRecordWidgetMapper::applyChanges()
{
	TSqlError error;
	error.RecordMessage = "QRecordWidgetMapper: Update: Error al actualizar los atributos";
...
/*! Inserta un nuevo registro con los valores de los widgets.
  \return Resultado de la acción.
*/
TSqlError QRecordWidgetMapper::saveNewRecord()
{
	TSqlError error;
	error.RecordMessage = "QRecordWidgetMapper: Insert: Error al insertar los atributos";
...
/*! Aplica en la base de datos las acciones del usuario.
  ...
  \return Resultado de la acción.
*/
TSqlError QRecordWidgetMapper::submit()
{
	TSqlError error;
	error.RecordMessage = "QRecordWidgetMapper: Submit: Error, No se ha establecido un modelo o no contiene registros";
...
TSqlError QRecordWidgetMapper::submitAll()
{
	TSqlError error;
...
void QRecordWidgetMapper::newRecord()
{
	newrecord=true;
...
void QRecordWidgetMapper::cancelNewRecord()
{
	if ( !newrecord )
		return;
...
void QRecordWidgetMapper::revert()
{
	actualRecord->revert();
	mapper();
}

void QRecordWidgetMapper::revertAll()
{
...
void QRecordWidgetMapper::cleanWidgets()
{
	foreach ( confMapper *cm , lstConfMappers )
		cleanWidget( cm->widget );
}
...
void QRecordWidgetMapper::modelToLstRecord(ModelBase* m)
{
...
void QRecordWidgetMapper::dataChanged(QWidget *widget)
{
...
void QRecordWidgetMapper::beginWidgetChangedByModel()
{
	disconnect(signalMapper,SIGNAL(mapped(QWidget*)),
			this,SLOT(dataChanged(QWidget*)));
}

void QRecordWidgetMapper::endWidgetChangedByModel()
{
	connect(signalMapper,SIGNAL(mapped(QWidget*)),
			this,SLOT(dataChanged(QWidget*)));
}

void QRecordWidgetMapper::model_modelReset()
{
	modelToLstRecord( model );
}
